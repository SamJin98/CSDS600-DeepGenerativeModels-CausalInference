...
model_path = 'checkpoint/000600.pt'
model_state=torch.load(model_path)
blocks['enc'].load_state_dict(model_state['encoder'])
blocks['dec'].load_state_dict(model_state['decoder'])
blocks['dag' ].load_state_dict(model_state['daglayer'])
blocks['attn'].load_state_dict(model_state['attention'])
blocks['mask_z'].load_state_dict(model_state['mask_z'])
blocks['mask_u'].load_state_dict(model_state['mask_u'])
...
imgs_path='data/celeba_test'

#dataset
test_dataset = dataload_withlabel(imgs_path,labels_4)
test_dataloader = Data.DataLoader(test_dataset, batch_size=args.batchsize, shuffle=False,drop_last=True)
...
for u, l in test_dataloader:
    u=u.to(device)

    intervene=torch.ones(l.size()).to(device)

    L, kl, rec, reconstructed_image,_ = negative_elbo_bound(args,blocks,u,intervene)
    
    save_image(u, 'test_result/reconstructed_image_true_{}.png', normalize = True,range=[0,255]) 
    save_image(reconstructed_image, 'test_result/reconstructed_image_{}.png', normalize = True) 